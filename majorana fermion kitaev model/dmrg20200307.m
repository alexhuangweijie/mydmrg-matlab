...
%for sweep = 1 : 10
while (sweep <100)||((sum(abs(PsiEnergy(:,1)-PsiEnergy(:,2)))>10^(-6))&&(sweep<30))
    for ii = 1 : 3%ii指标反映对应的扫的时候的移动方向 奇数向右 偶数向左
        ...
        while counter<1000000
            ...
            %测量
            [ measure_temp1 , measure_temp2 ] = Measure ( Block_L , Free_site_L , Free_site_R , Block_R , Psi );
            %求密度矩阵 约化密度矩阵 求变换矩阵 返回变换矩阵
            [ Block_L , Block_R , transform1 , transform2] = Rotate_operator( Block_L , Block_R , Psi  , noftk );
            %存储变换矩阵
            if mod(ii,2)==1
            ...
            elseif ii == 2
                ...
                measure1(sites_in_system+2, sweep) = abs(measure_temp1);
            end
...
%             if iii==1||iii==2
%                 Psi = Uoperator_hubbard( Block_L , Free_site_L , Free_site_R , Block_R , j , deltaT ,Psi);
%             else
%                 U = 1;
%             end
%             %Psi = U * Psi;
...
%             if iii == 3
%                 [ measure_temp1 , measure_temp2 ] = Measure ( Block_L , Free_site_L , Free_site_R , Block_R , Psi );
%                 measure2(sites_in_system+2, sweep2) = measure_temp1;
%             end
%             %读取变换矩阵
...
figure(1)
plot(measure1(3:end,33));
%hold on
%plot(measure2(2:end,1:end))
%figure(2)
%plot(PsiEnergy(:,1)-PsiEnergy(:,2))
%figure(2)
% for index = 1:13
%     figure(index)
