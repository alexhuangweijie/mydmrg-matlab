function [V, c1, j]=svd_L(H1 ,H4 ,J ,Psi)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%svd  using lanczos method to H*H^T 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
format long
D = H1.basis_size*H4.basis_size;c = ones(D,1);
%Iteration with j=0
if length(Psi) ==D
    r0 = Psi;
else
...
% Iteration with j>=2
j=1;b(2)=9;c0 = rand;c1 = rand;
T1 = zeros(j,j);T1(1,1)=a(1);
while (abs(c1-c0)>10^(-8))&&(j<500)
%while j<D
    q(:,j+1)=r(:,j)/b(j);
    j = j+1;
