        ...
        if sequence(1)==0
            ...
            if tensors(2)==0
                ...
                if ~isequal(sort(contractionIndices),sort(commonIndices))
                    ...
                    tdims = [tdims size(tensorList{tensors(2)}) ones(1,numel(legLinks{tensors(2)})-ndims(tensorList{tensors(2)}))]; %#ok
...
function [tensorList legLinks sequence warnedLegs] = zisOuterProduct(tensorList,legLinks,sequence,warnedLegs)
    ...
    while numel(OPsizes)>1
        ...
        if ~isempty(commonIndices)
            ...
            tdims = [tdims size(tensorList{OPlist(ix(2))}) ones(1,numel(legLinks{OPlist(ix(2))})-ndims(tensorList{OPlist(ix(2))}))]; %#ok
